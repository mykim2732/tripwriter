...
export default function QaPage() {
  ...
  const items: QaItem[] = [
    ...
    { label: "첫 실행 온보딩", status: "ready", description: "/onboarding에서 사진 넣기, AI 글 작성, 복사/발행 3단계가 로그인 전에도 이해되는지 확인하세요." },
    { label: "로그인 전 샘플 데모", status: "ready", description: "/demo에서 여행, 리뷰, 상세페이지 mock 결과 3개가 실제 OpenAI 호출 없이 표시되는지 확인하세요." },
    { label: "원탭 작성 polish", status: "ready", description: "/write 빠른 작성 모드에서 만들기 버튼까지 모바일 첫 흐름 안에 자연스럽게 도달하는지 확인하세요." },
    { label: "발행 복사 fallback", status: "ready", description: "/publish/[id]에서 HTML 복사 실패 시 텍스트, 대표사진, 이미지 URL, 링크, 태그가 자동 fallback 복사되는지 확인하세요." },
    { label: "복사 결과 미리보기", status: "ready", description: "발행 화면의 복사 결과 미리보기 카드에서 문자 수, 이미지 URL 수, HTML 포함 수가 보이는지 확인하세요." },
    { label: "자동 임시저장 복구", status: "ready", description: "/write 입력 후 새로고침했을 때 localStorage 복구 카드가 뜨고 복구/나중에 버튼이 동작하는지 확인하세요." },
    { label: "저장 실패 재시도", status: "ready", description: "저장 실패 메시지 아래 저장 다시 시도 버튼이 표시되고 같은 저장 흐름을 재실행하는지 확인하세요." },
    { label: "모바일 키보드", status: "ready", description: "iOS Safari 기준 입력 중 하단 내비, sticky 만들기 버튼, toast가 키보드와 safe-area에 가려지지 않는지 확인하세요." },
    { label: "크레딧 부족 UX", status: "ready", description: "크레딧 부족 카드에서 입력 유지 안내, 무료로 채우기, 업그레이드, 광고 리워드 안내 CTA가 보이는지 확인하세요." },
    { label: "첫 생성 피드백", status: "ready", description: "첫 생성 완료 후 별점/한줄 피드백 카드가 표시되고 localStorage 제어로 너무 자주 뜨지 않는지 확인하세요." },
    { label: "성능 패스", status: "ready", description: "고급 리뷰 참고 패널이 필요할 때 lazy load되고 이미지 preview가 lazy/async로 렌더링되는지 확인하세요." },
