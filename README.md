...
- AI 글 생성: 빠른 작성 모드에서 사진, 장소/상품명, 한 줄 메모, 내 말투만으로 초안 생성
- 원탭 생성: 대표사진, 키워드, 말투, 소제목, SEO, 사진배치를 자동 설정
- AI 구성 계획: 초안 생성 전에 독자, 사진 흐름, 섹션, SEO/디자인 방향을 먼저 설계
- AI 디자인: 글 다듬기, 소제목, 사진 설명, 이미지 꾸미기 추천
- AI 품질 검수: 사람이 쓴 느낌, AI 티, 반복, 과장, 경험 없는 단정, 리뷰/경험 혼동을 생성 직후 점검
- 공통 편집기: BlogEditor, DetailEditor, ReviewEditor와 FloatingEditorToolbar
- 사진 관리: 업로드, 드래그앤드롭, 순서 변경, 설명 수정, AI 사진 분석
- 사진 타임라인: 도착, 입장, 메인, 디테일, 마무리 흐름으로 AI 추천 순서 표시
- 사진 기반 다시 쓰기: 대표사진, 본문 사진, 마무리 사진 흐름으로 초안 재작성
- 리뷰 참고: 장소/상품명 검색, 리뷰 미리보기, 선택 포인트 반영 흐름으로 안전하게 초안에 반영
...
- AI 대표사진 후보: 클릭률 기준 3개 후보와 CTR 예상 표시
...
- 첫 생성 피드백: 생성 완료 후 별점/한줄 피드백을 localStorage로 과하지 않게 요청
- 임시저장 복구: `/write` 입력과 생성 초안을 localStorage에 자동 저장하고 새로고침 후 복구 안내
- 발행 복사 안정화: HTML 복사 실패 시 텍스트, 대표사진, 이미지 URL, 링크, 태그 fallback 자동 복사
- 로그인: Supabase Auth 이메일 회원가입/로그인/로그아웃
- 크레딧: profiles/credit_logs 기반 차감 준비 및 로그인 사용자 실제 차감 연결
- 운영 점검: `/qa` 체크리스트 페이지
- Labs: SEO, 첨부파일, 구성 보기, 직접 리뷰 메모 등 고급/실험 기능은 접힌 영역에서 관리
...
- Google Places 리뷰 미리보기는 `GOOGLE_PLACES_API_KEY`가 있을 때 공식 API 응답 범위 안에서만 사용하며, 키가 없으면 mock/search-link fallback을 사용합니다.
...
## Sprint 141~150 Simplification Notes

- `/write` now starts in quick creation mode with photos, place/product name, one-line memo, voice, and create action visible first.
- Advanced fields such as SEO keywords, links, attachments, manual review notes, detailed tone, and content planning stay folded by default.
- Review research now centers on place/product search, review preview cards, and explicit "use in writing" toggles.
- Google Places can provide rating, review count, and up to 5 official API review snippets when `GOOGLE_PLACES_API_KEY` is configured; otherwise Posty AI uses mock/search-link fallback.
- Selected review preview points are passed into review insight analysis and writing prompts, while user experience and external review references are kept separate.
- Smart defaults recommend photo-first writing, review reference, recent writing style, cover photo, and generated keywords with less manual input.
- Quality review now includes a human-like writing score and checks AI-like phrases, repetition, overstatement, unsupported experience claims, and review/experience confusion.

## Sprint 151~160 MVP Release Candidate Notes

- One Tap Creation lets users create from photos, place/product name, and one-line memo while Posty AI fills title, keywords, voice, SEO, and photo placement.
- Photo Timeline shows AI-recommended story order: arrival, entrance, main, detail, and closing.
- AI Designer 3.0 strengthens diary-style visual overlays with white hand-drawn marks, arrows, hearts, stars, handwritten memo, tape, polaroid, and soft paper style.
- Human Emotion Writer adds emotional arc, honest hesitation, unexpected moments, sentence-length variety, and less repetitive blogger phrasing.
- AI Cover Selection shows three CTR-oriented cover photo candidates.
- Result Preview appears before generation with cover photo, title, subtitles, tone, estimated length, and reading time.
- Failure Recovery creates a local temporary draft when OpenAI, photo analysis, credit, or timeout failures happen.
- Mobile polish keeps the create action thumb-friendly and adds safer bottom spacing.
- Empty states now show example photo flow and review reference examples.
- Advanced and experimental controls are grouped under Labs so the default path stays simple.

## Sprint 161~170 Beta UX Notes

- First-run onboarding explains the shortest flow: add photos, let AI write, then copy or publish.
- `/demo` provides login-free mock samples for travel, review, and detail-page creation.
- `/write` keeps one-tap creation compact, adds local autosave recovery, and provides a retry action after save failure.
- `/publish/[id]` now shows a copy-result preview and automatically falls back to text plus image URLs when HTML clipboard copy is blocked.
- Mobile keyboard and safe-area spacing were polished for sticky create actions, bottom navigation, and toast placement.
- Credit-empty UX now guides users toward rewards, pricing, and ad reward guidance without losing their inputs.
- First generation can ask for a lightweight beta rating and one-line feedback, throttled with localStorage.
- Advanced review research loads lazily and image previews use lazy/async rendering attributes.
